    ...
    public class P09T1Grader : ITaskGrader
    {
        ...
        public TaskResult Grade(ExcelWorksheet studentSheet)
        {
            ...
            try
            {
                ...
                {
                    result.Errors.Add("? Không tìm th?y bi?u d?");
                    return result;
                }

                decimal score = 0;

                // Rule 1: Plot Area có pattern fill 10% (2.5 di?m)
                ...
                if (hasPlotFill)
                {
                    ...
                    {
                        score += 1m;
                        result.Errors.Add($"? Plot area có pattern fill nhung type là '{plotPattern}' (mong d?i: pct10)");
                    }
                }
                else
                {
                    result.Errors.Add("? Plot area chua có pattern fill");
                }

                // Rule 2: Chart Area có pattern fill 50% (2.5 di?m)
                ...
                if (hasChartFill)
                {
                    ...
                    {
                        score += 1m;
                        result.Errors.Add($"? Chart area có pattern fill nhung type là '{chartPattern}' (mong d?i: pct50)");
                    }
                }
                else
                {
                    result.Errors.Add("? Chart area chua có pattern fill");
                }
