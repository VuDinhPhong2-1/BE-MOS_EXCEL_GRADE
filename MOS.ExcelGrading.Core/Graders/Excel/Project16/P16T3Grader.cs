    ...
    public class P16T3Grader : ITaskGrader
    {
        ...
        public TaskResult Grade(ExcelWorksheet studentSheet)
        {
            ...
            try
            {
                ...
                {
                    result.Errors.Add("Không tìm thấy cột 'Quantity'.");
                    return result;
                }

                if (dataStart <= 0 || dataEnd < dataStart)
                {
                    result.Errors.Add("Không xác định được vung dữ liệu cột Quantity.");
                    return result;
                }
                ...
                if (matchedRule == null)
                {
                    ...
                    {
                        result.Errors.Add($"Không tìm thấy icon set tren range {expectedRange}.");
                    }
                    else
                    {
                        var types = string.Join(", ", targetedRules.Select(r => r.Type.ToString()).Distinct(StringComparer.OrdinalIgnoreCase));
                        result.Errors.Add($"Co conditional formatting tren range {expectedRange} nhung không phải 3-icon set (types: {types}).");
                    }
                ...
                score += 2m;
                result.Details.Add($"Tìm thấy icon set tren range {expectedRange}.");
                ...
                {
                    result.Errors.Add($"Icon set/nguong chưa đúng. iconSet='{iconSetName}', icon1={type1}:{val1}, icon2={type2}:{val2}, icon3={type3}:{val3}.");
                }
