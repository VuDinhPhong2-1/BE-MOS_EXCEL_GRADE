    ...
    public class P09T6Grader : ITaskGrader
    {
        ...
        public TaskResult Grade(ExcelWorksheet studentSheet)
        {
            ...
            try
            {
                ...
                if (ws == null)
                {
                    result.Errors.Add("Không tìm th?y sheet 'Farmers & Market'.");
                    return result;
                }
                ...
                {
                    result.Errors.Add("Không tìm th?y chart Pie 3D tren sheet Farmers & Market.");
                    return result;
                }

                score += 2m;
                result.Details.Add($"Tìm th?y {pieCharts.Count} chart Pie 3D.");
                ...
                {
                    score += 3m;
                    result.Details.Add("Vung d? li?u dung: Category B13:B18 va Value F13:F18.");
                }
                else
                {
                    var firstSeries = targetChart.Series.FirstOrDefault();
                    result.Errors.Add(
                        $"Vung d? li?u chua dúng. X='{firstSeries?.XSeries}', Y='{firstSeries?.Series}'.");
                }
                ...
                else
                {
                    ...
                    result.Errors.Add($"Vi tri chart chua dúng. Hi?n t?i: {currentBounds}, mong d?i: J2:P15.");
                }
