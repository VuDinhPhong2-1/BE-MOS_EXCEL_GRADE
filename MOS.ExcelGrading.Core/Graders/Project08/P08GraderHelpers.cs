...
{
    internal static class P08GraderHelpers
    {
        ...
        public static decimal ToDecimal(object? value, string? text = null)
        {
            ...
            if (decimal.TryParse(text ?? string.Empty, NumberStyles.Any, CultureInfo.InvariantCulture, out var parsedInvariant))
            {
            ...
            var cleaned = (text ?? string.Empty)
                ...
                .Replace(",", string.Empty, StringComparison.Ordinal)
                .Trim();
            return decimal.TryParse(cleaned, NumberStyles.Any, CultureInfo.InvariantCulture, out var cleanedParsed)
                ? cleanedParsed
                : 0m;
        }
    }
