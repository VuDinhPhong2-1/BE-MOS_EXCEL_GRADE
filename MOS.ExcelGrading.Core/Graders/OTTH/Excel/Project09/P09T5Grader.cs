        ...
        public TaskResult Grade(ExcelWorksheet studentSheet)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        result.Errors.Add($"outlineLevelRow/cau truc subtotal chua dúng. Hi?n t?i outlineLevelRow='{outlineLevelText}'.");
                    }
                }
                ...
                {
                    score += 1m;
                    result.Details.Add("S? lu?ng manual row break hop le (3 hoac 4).");
                }
                else
                {
                    result.Errors.Add(
                        $"S? lu?ng manual row break chua dúng. detected={detectedManualBreakCount}, mong d?i 3 hoac 4.");
                }
                ...
                {
                    result.Errors.Add(
                        $"Row break id chua dúng. Hi?n t?i: {string.Join(", ", actualBreakIds.OrderBy(x => x))}. Mong doi quanh cac moc 46, 103, 156 (va tuy chon 201).");
                }
                ...
                foreach (var pair in expectedFormulas)
                {
                    ...
                    {
                        result.Errors.Add(
                            $"{pair.Key} sai cong thuc. Hi?n t?i: '{ws.Cells[pair.Key].Formula}'.");
                    }
                }
