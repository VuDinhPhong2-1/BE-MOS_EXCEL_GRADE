...
{
    internal static class P07GraderHelpers
    {
        ...
        {
            var text = (address ?? string.Empty).Replace("$", string.Empty, StringComparison.Ordinal).Trim();
        ...
        public static decimal ToDecimal(object? value, string? text = null)
        {
            ...
            var cleaned = (text ?? string.Empty)
                ...
                .Replace(",", string.Empty, StringComparison.Ordinal)
                .Trim();
            return decimal.TryParse(cleaned, out var cleanParsed) ? cleanParsed : 0m;
        }
    }
}
